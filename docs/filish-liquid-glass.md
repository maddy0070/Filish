...
> **Extended by V3.1 and V3.2.** The material model below is intact and still
> governs **chrome** — sheets, action bars, ledgers, confirmations. It no
> longer governs content: at rest a file has no surface at all, and the band
> became a variable-width *mark* in a leading gutter, carrying magnitude and,
> for media, a sliver of the file's own content.
>
> `Modifier.lens()` below is for chrome. `Modifier.worldObject()` is for
> content. See [visual-language.md](visual-language.md), which supersedes this
> document on composition, the environment, light and light mode.
